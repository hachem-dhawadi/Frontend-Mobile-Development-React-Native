...
const ServiceCategory = () => {
  ...
  useEffect(() => {
    ...
    axios.get('/sanctum/csrf-cookie').then(response => {
      axios.get(`/api/catserv/${route.params.id}`).then((res) => {
        ...
        onPress={() => {
          if (item.active === 0) {
            Alert.alert(
              ...
              "Sorry, this service is currently not available. Please try again later.",
              [{ text: "OK", onPress: () => console.log("OK Pressed") }]
            );
          } else if (item.active === 1) {
            navigation.navigate("RoomListByService", { id: item.id })
            console.log(item.id);
          }
        }}
      >
        ...
        <View >
          ...
          <Text style={styles.soustitle}>Owner : {item.user.name}</Text>
          <TouchableOpacity onPress={() => Linking.openURL('geo:0,0?q=' + item.adresse)}>
            <Text style={styles.soustitle2}>Gps location: {item.location}</Text>
          </TouchableOpacity>
